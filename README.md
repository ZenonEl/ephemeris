...
```
/ephemeris:handoff    закрыть смену: зафиксировать → обновить дейлик → дописать передачу
...
Настоящие имена организаций, проектов, людей и внутренних трекеров в
репозиторий не попадают — ни в документации, ни в примерах, ни в сообщениях
коммитов. Адрес репы дейликов и assignee живут в `~/.config/ephemeris/daily.conf`.

Заглушки — те же, что в mnemo: slug `priyomka`, `sozvony`; внешний трекер
`<owner>/<репо>#110`; идентификаторы записей `i004`, `q007`.
